				...
				switch (target[0].nodeName.toLowerCase()){
					...
					case 'td':
						if (target.is('.day') && !target.is('.disabled')){
							...
							var checkdate = checkcorrectdate(UTCDate(year, month, day));
							if(checkdate){
							    var currentMonth = month+1;
								$("input[name='selectedDate'").val(year+'-'+currentMonth+'-'+day).trigger('change');
								$('#datetimeshipping').css('display','block');
								$('#datetimelink').css('display','none');
