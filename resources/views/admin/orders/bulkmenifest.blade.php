...
<table class="table-main">
	<tbody>
		<tr>
			<td style="text-align: left;">{{$website_name}} (Merchant ID : {{Auth::user()->id}})</td>
			<td>BLUE DART Pickup List</td>
			<td style="text-align: right;">Print Date: {{$date}} hours</td>
		</tr>
	</tbody>
</table>
		...
		@foreach($orders as $order)
			...
			@foreach($order->products as $product)
				...
				<tr>
					...
					<td>--</td>
					<td>{{$order->id}}</td>
					<td style="max-width: 300px;">{{substr($product->product_name,0,50)}}</td>
...
<table class="table-main">
	<tbody>
		<tr>
			...
			<td style="text-align: right;font-size:12px;">Total Shipments: {{count($orders)}} orders</td>
		</tr>
	</tbody>
</table>
