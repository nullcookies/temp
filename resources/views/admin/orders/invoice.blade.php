...
<p class="m-5">Invoice for S/{{$user->id}}/{{$order->id}} {{date('D d-M-Y')}}</p>
<h3 class="m-5">Retail/TaxInvoice/Cash Memorandum</h3>
<br/>
<h3 class="m-5">Sold By</h3>
<p class="m-5">{{$user->subdomain}}.techturtle.in</p>
<!-- <p class="m-5">Prathamesh Complex, Building No. H, Opp. Vatika Restaurant</p> -->
<p class="m-5">{{$user->city}}</p>
<p class="m-5">{{$user->state}}, India</p>
<!-- <p class="m-5">Maharashtra, India</p> -->
<br/><br/>
<p class="m-5">VAT/TIN Number: {{DB::table('user_business_detail')->where('user_id', $user->id)->select('tin')->first() ? DB::table('user_business_detail')->where('user_id', $user->id)->select('tin')->first()->tin : ''}}</p>
	...
	<tbody>
		...
		<tr>
			<td class="caps">
				...
				<p>{{$order->customerCity}}-{{$order->customerPostCode}}</p>
		...
		<tr>
			<th>Order ID: S/{{$user->id}}/{{$order->id}}</th>
...
<table class="table">
	...
	@foreach($order->products as $product)
		<tr class="vtop">
			...
			<th width="30%" style="text-align:left;">{{$product->product_name}}</th>
			<td width="8%" style="text-align:left;">Rs. {{$order->orderAmount - ($order->orderAmount*15)/100}}</td>
			<td width="8%" style="text-align:left;"></td>
			<td width="8%" style="text-align:left;">Rs. {{$order->orderAmount}}</td>
			<td width="8%" style="text-align:left;">Service Tax</td>
			<td width="8%" style="text-align:left;">15%</td>
			<td width="10%" style="text-align:left;">Rs. {{($order->orderAmount*15)/100}}</td>
		</tr>
		<tr>
			<td width="2.5%" style="text-align:left;"></td>
			<td width="30%" style="text-align:left;">Shipping</td>
			<td>Rs. {{$order->shippingCharge - ($order->shippingCharge*15)/100}}</td>
			<td></td>
			<td>Rs. {{$order->shippingCharge}}</td>
			<td>Service Tax</td>
			<td>15%</td>
			<td>Rs. {{($order->shippingCharge*15)/100}}</td>
		</tr>
	@endforeach
	</tbody>
</table>
	...
	<table class="table">
		<tbody>		
			<tr class="vtop">
				...
				<td width="8%" style="text-align:left;">Rs. {{$order->orderAmount - ($order->orderAmount*15)/100 + $order->shippingCharge - ($order->shippingCharge*15)/100}}</td>
				<td width="8%" style="text-align:left;"></td>
				<td width="8%" style="text-align:left;">Rs. {{$order->orderAmount + $order->shippingCharge}}</td>
				<td width="8%" style="text-align:left;">Service Tax</td>
				<td width="8%" style="text-align:left;">15%</td>
				<td width="8%" style="text-align:left;">Rs. {{($order->orderAmount*15)/100 + ($order->shippingCharge*15)/100}}</td>
			</tr>
		</tbody>
	</table>
</div>
		...
		<tbody>
			<tr>
				...
				<td>
					...
					<div class="signbox"><img width="300" height="100" src="{{strlen($user->signature) ? $user->signature : url('/no_image.png') }}"></div>
				</td>
			</tr>
		</tbody>
	</table>
...
<div class="regadd">
	<h4>Registered Address for {{$user->subdomain}}.techturtle.in, {{$user->city}}, {{$user->state}}, India</h4>
</div>
	...
	<table class="table">
		<tbody>
			<tr>
				...
				<td>
					<h4 style="text-align:center;">Purchase made on <b>{{$user->subdomain}}.techturtle.in</b><!-- <img src="http://media.corporate-ir.net/media_files/IROL/17/176060/img/logos/amazon_logo_RGB.jpg" class="logo-image" width="100">--></h4> 
