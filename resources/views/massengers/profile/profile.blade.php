...
<!-- cropie -->
<script
  src="{{asset('massengers/js/jquery-ui.min.js')}}"></script>
<script>
  $( function() {
...
<link href="{{asset('massengers/css/jquery-ui.theme.min.css')}}" rel="stylesheet" />
<style>
				...
				<div class="tab-content">
					...
					<div id="profile" class="tab-pane fade in active">
						...
						<div class="row pd-10">
							<div class="col-md-4 col-sm-6 profile-profile">
								...
								<img width="50%" src="{{$profile_image}}"> 
								<label style="cursor:pointer;" data-toggle="modal" data-target="#uploadprofilepic">Update Picture</label>
								<p>Mobile No. : <span class="c-red">+91-{{Auth::user()->mobile}}</span></p>
								<p>Mail Id : <span class="c-red">{{Auth::user()->email}}</span></p>
								<p>DOB: <span class="c-red">{{Carbon\Carbon::parse(Auth::user()->dob)->format('D d-M-Y')}}</span></p>
							</div>	
						</div>
					</div>
						...
						@foreach($addresses as $address)
							<div class="col-md-4 col-sm-6 profile-address">
							    <div class="profile-address-box">
    								...
    								<p>Mobile No. : <span class="c-red">+91-{{$address->mobile}}</span></p>
    								<p class="mb-10">Email : <span class="c-red">{{$address->email}}</span></p>
								</div>
							</div>
						@endforeach	
						</div>	
						...
						<table class="table profile-orders roboto-light">
							...
							<tbody>
								...
								<tr>
									...
									<td>@if($order->is_delivered == 'yes') {{Carbon\Carbon::parse($order->delivery_time)->format('D d-m-Y')}} @else -- @endif</td>
									<td>{{$order->status}}</td>
								</tr>
								@endforeach
							</tbody>
					...
					<div id="rstpwd" class="tab-pane fade">
						<h3 class="c-red">Change Password</h3>
                            ...
                            @endif
						    <form action="{{url('/change-password')}}" method="post" role="form" class="formchangepwd">
                                {{csrf_field()}}
	...
	<div class="modal-dialog">
		<div class="modal-content newsletter-content">
			<div class="modal-body text-center">
				...
				<form id="updateprofileform" method="post" action="{{url('/updateprofile')}}">
				    {!! csrf_field() !!}
				    <div id="updateprofilemessage"></div>
					<input type="text" class="news-input" name="name" value="{{Auth::user()->name}}" placeholder="Enter Your Name">
					<input type="text" class="news-input" name="mobile" value="{{Auth::user()->mobile}}" placeholder="Enter Your Mobile Number">
					<input type="text" class="news-input" name="dob" value="{{Carbon\Carbon::parse(Auth::user()->dob)->format('m/d/Y')}}" id="datepicker" placeholder="Enter Your Date of Birth">
